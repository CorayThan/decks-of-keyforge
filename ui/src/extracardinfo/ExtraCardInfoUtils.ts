...
export class ExtraCardInfoUtils {
    ...
    static minAERC = (type: CardType, info: ExtraCardInfo): number => {
        return info.amberControl + info.artifactControl + info.creatureControl + info.creatureProtection
            + info.disruption + info.effectivePower + info.efficiency
    ...
    static maxAERC = (type: CardType, info: ExtraCardInfo): number => {
        return (info.amberControlMax ?? info.amberControl) + (info.artifactControlMax ?? info.artifactControl)
            ...
            + (info.disruptionMax ?? info.disruption) + (info.efficiencyMax ?? info.effectivePower) + (info.efficiencyMax ?? info.efficiency)
