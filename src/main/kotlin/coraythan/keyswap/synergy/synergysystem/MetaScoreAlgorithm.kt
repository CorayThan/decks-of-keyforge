...
object MetaScoreAlgorithm {
    fun generateMetaScores(
        aemberControl: Double,
        creatureControl: Double,
        traitsMap: Map<SynergyTrait, MatchSynergiesToTraits>
    ): Map<String, Double> {
        ...
        val boardWipeScoreByHouse =
            traitsMap[SynergyTrait.boardClear]?.traitValues
                ...
                ?.groupBy { it.house!! }
                ?.map {
                    it.key to it.value.sumOf { synTraitValueWithHouse ->
