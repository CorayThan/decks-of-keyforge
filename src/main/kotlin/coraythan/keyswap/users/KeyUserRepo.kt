...
interface KeyUserRepo : JpaRepository<KeyUser, UUID>, QuerydslPredicateExecutor<KeyUser> {
    ...
    fun findByUsernameContainsIgnoreCase(username: String): List<KeyUser>
    ...
    @Modifying
    @Query("UPDATE KeyUser keyUser SET keyUser.storeIconKey = ?1 WHERE keyUser.id = ?2")
    fun setStoreIconKey(storeIconKey: String?, userId: UUID)

    @Modifying
    @Query("UPDATE KeyUser keyUser SET keyUser.storeBannerKey = ?1 WHERE keyUser.id = ?2")
    fun setStoreBannerKey(storeBannerKey: String?, userId: UUID)
...
interface KeyUserSearchProjectionRepo: CrudRepository<KeyUser, UUID> {
    fun findByUsernameIgnoreCase(username: String): KeyUserDeckSearchInfo?
}

data class KeyUserDeckSearchInfo(
        val id: UUID,
        val teamId: UUID?,
